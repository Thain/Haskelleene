...
main = hspec $ do
  describe "Examples" $ do
    ...
    it "DA to reg should give the same result" $
      acceptDA wikiDA 0 [A] == regexAutAccept (dautToReg wikiDA 0) [A] `shouldBe` True

regexAutAccept :: (Alphabet l) => Regex l -> [l] -> Bool
regexAutAccept reg input = uncurry ndautAccept (fromReg reg) input
\end{code}
