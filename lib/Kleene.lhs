...
import Basics ( Alphabet(..) )      -- contains all of our utility functions
import Automata
    ( NDetAut, DetAut(delta, states, accept), AutData(..), encodeNA )    -- contains automaton definitions/conversions
import Regex ( altList, seqList, simplifyRegex, Regex(..) )       -- contains regex definitions
...
In the end, we also include a more direct recipe of converting a \emph{deterministic} automaton to a regular expression, inspired by~\cite{book_marcelo}. The idea is to implement a function \texttt{dautToRegSub} as follows. For any deterministic automaton, with a \emph{subset} $Q$ of its states, an input state $q$ and an output state $q'$, this function output a regular expression $r^Q_{q,q'}$ that accepts those strings $u$ satisfying the following condition: With input $u$, starting from $q$ the automaton traverses states \emph{in $Q$} and terminates at $q'$. The additional parameter $Q$ allows us to define this function recursively.
...
dautToReg daut s = simplifyRegex $ 
  foldr (Alt . dautToRegSub daut s (states daut)) Empty $ accept daut
